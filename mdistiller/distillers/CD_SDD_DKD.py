...
def cluster_loss(patch_student, patch_teacher, gamma, temperature=1):
    """
    patch_student: [B, C, P]，P 是 patch 数量
    patch_teacher: [B, C, P]
    gamma: loss 缩放系数
    ...
    # L2 归一化
    patch_student = F.normalize(patch_student, dim=1)
    patch_teacher = F.normalize(patch_teacher, dim=1)

    B, C, P = patch_student.shape

    # 定义 KL loss 函数
    def compute_kl_loss(sim_s, sim_t, temperature):
        return F.kl_div(
            F.log_softmax(sim_s / temperature, dim=-1),
            F.softmax(sim_t / temperature, dim=-1),
            reduction='batchmean'
        )
    ...
    if P == 1:
        ...
        groups = [(0, 1), (1, 17)]      # 1 + 16
        weights = [1.0 / 4, 3.0 / 4]
    elif P == 21:
        groups = [(0, 1), (1, 5), (5, 21)]  # 1 + 4 + 16
        weights = [1.0 / 6, 2.0 / 6, 3.0 / 6]
    else:
        raise NotImplementedError(f"Patch 数量 {P} 不支持")

    total_loss = 0.0

    for (start, end), weight in zip(groups, weights):
        # 提取当前组的 patch
        ...
        # 计算 KL loss
        # loss = compute_kl_loss(sim_s, sim_t, temperature)
        loss = compute_kl_loss(sim_s, sim_t, temperature=2) * (temperature ** 2)

        # 加权累加
        total_loss += weight * loss

    return gamma * total_loss
# def cluster_loss(patch_student, patch_teacher, gamma, temperature=1):
#     # patch_student: [B, C, P]
#     # patch_teacher: [B, C, P]
...
#     # 计算相似度矩阵
#     sim_student = torch.bmm(patch_student.transpose(1, 2), patch_student)
#     sim_teacher = torch.bmm(patch_teacher.transpose(1, 2), patch_teacher)

#     # 使用 KL 散度对 patch 间相似度分布进行对齐
#     loss = F.kl_div(
#         F.log_softmax(sim_student / temperature, dim=-1),
#         F.softmax(sim_teacher / temperature, dim=-1),
#         reduction='batchmean'
#     )
...
class CD_SDD_DKD(Distiller):
    """Decoupled Knowledge Distillation(CVPR 2022), Scale Decoupled Distillation(CVPR 2024)"""

    def __init__(self, student, teacher, cfg):
        super(CD_SDD_DKD, self).__init__(student, teacher)
